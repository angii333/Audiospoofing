...
class Generator(nn.Module):
    ...
    def forward(self, x, z):
        ...
        z = torch.nn.functional.pad(z, (0, -1))  # Pad in the last dimension 
        ...
        d10 = torch.nn.functional.pad(d10, (0, 1))  # Pad the last dimension to match e10 size
        # dx_c : concatenated with skip-connected layer's output & passed nonlinear layer
        d10_c = self.dec10_nl(torch.cat((d10, e10), dim=1))
        d9 = self.dec9(d10_c)
        d9 = torch.nn.functional.pad(d9, (0, 1))  # Pad the last dimension to match e10 size
        d9_c = self.dec9_nl(torch.cat((d9, e9), dim=1))
        d8 = self.dec8(d9_c)
        #d8 = torch.nn.functional.pad(d8, (0, 1))  # Pad the last dimension to match e10 size
        d8_c = self.dec8_nl(torch.cat((d8, e8), dim=1))
        d7 = self.dec7(d8_c)
        d7 = torch.nn.functional.pad(d7, (0, 1))  # Pad the last dimension to match e10 size
        d7_c = self.dec7_nl(torch.cat((d7, e7), dim=1))
        d6 = self.dec6(d7_c)
        #d6 = torch.nn.functional.pad(d6, (0, 1))  # Pad the last dimension to match e10 size
        d6_c = self.dec6_nl(torch.cat((d6, e6), dim=1))
        d5 = self.dec5(d6_c)
        #d5= torch.nn.functional.pad(d5, (0, 1))  # Pad the last dimension to match e10 size
        d5_c = self.dec5_nl(torch.cat((d5, e5), dim=1))
        d4 = self.dec4(d5_c)
        #d4 = torch.nn.functional.pad(d4, (0, 1))  # Pad the last dimension to match e10 size
        d4_c = self.dec4_nl(torch.cat((d4, e4), dim=1))
        d3 = self.dec3(d4_c)
        #d3 = torch.nn.functional.pad(d3, (0, 1))  # Pad the last dimension to match e10 size
        d3_c = self.dec3_nl(torch.cat((d3, e3), dim=1))
        d2 = self.dec2(d3_c)
        #d2 = torch.nn.functional.pad(d2, (0, 1))  # Pad the last dimension to match e10 size
        d2_c = self.dec2_nl(torch.cat((d2, e2), dim=1))
        d1 = self.dec1(d2_c)
        #d1 = torch.nn.functional.pad(d1, (0, 1))  # Pad the last dimension to match e10 size
